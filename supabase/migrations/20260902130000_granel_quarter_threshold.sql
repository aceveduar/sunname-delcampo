-- create_sale (el cálculo que de verdad cobra) seguía con el quiebre
-- de tarifa granel en 1kg, mientras que el cliente (lib/granel.ts) ya
-- se corrigió a 1/4 kg (250g) con las hojas de precio reales del dueño
-- del 21 de agosto -- confirmado que el cuarto es SIEMPRE exacto
-- precio_kilo ÷ 4 en los 27 productos revisados. Como create_sale
-- rechaza la venta si el pago no coincide exacto con lo que él mismo
-- calcula, este desface significaba que cualquier venta a granel entre
-- 250g y 1kg fallaba por completo ("los pagos no coinciden con el
-- subtotal") desde que se desplegó el cambio del lado del cliente --
-- no solo cobraba distinto, tumbaba la venta.
...
begin
  ...
  loop
    ...
    if v_sold_by_weight then
      -- Quiebre en 1/4 kg (250g), no en 1kg: confirmado con las hojas
      -- de precio reales del dueño que el cuarto es siempre exacto
      -- precio_kilo ÷ 4, así que aplicar la tarifa de kilo desde 250g
      -- ya da el precio real del cuarto sin guardar una tarifa aparte.
